    ...
    <div className="rounded-2xl border border-border bg-[#080d1a]/25 backdrop-blur-xl shadow-2xl overflow-hidden text-sm text-foreground ring-1 ring-white/5">
      ...
      <div className="relative px-6 py-5 flex gap-6 items-start border-b border-border">
        <div
          ...
          aria-hidden
        />
        <div className="relative shrink-0 w-24 h-24 rounded-xl bg-gray-50 dark:bg-white/5 flex items-center justify-center overflow-hidden border border-border shadow-inner">
          {draft.company.logoBase64 ? (
            ...
            <span className="text-xs text-secondary text-center px-1">Logo</span>
          )}
        </div>
        <div className="relative flex-1 min-w-0 space-y-1">
          <div className="font-bold text-lg leading-tight text-foreground tracking-tight">
          ...
          {draft.company.siret && (
            <div className="text-xs text-secondary">SIRET : {draft.company.siret}</div>
          )}
          {draft.company.tvaIntra && (
            <div className="text-xs text-secondary">TVA : {draft.company.tvaIntra}</div>
          )}
      ...
      <div className="px-6 py-6 space-y-6 bg-gradient-to-b from-black/20 to-black/35">
        ...
        <div className="rounded-xl border border-border bg-[#080d1a]/25 backdrop-blur-sm p-4 shadow-inner">
          ...
          <div className="text-subtitle whitespace-pre-line mt-1 text-sm">
            {draft.client.adresse || "—"}
          </div>
          {(draft.client.email || draft.client.phone) && (
            <div className="mt-2 text-xs text-secondary space-y-0.5 border-t border-border pt-2">
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {draft.lignes.map((l, i) => (
                ...
                >
                  <td className="p-3 align-top text-foreground">{l.description || "—"}</td>
                  <td className="p-3 text-right text-subtitle">{l.quantity}</td>
                  <td className="p-3 text-center text-subtitle">{uniteLabel(l.unite)}</td>
        ...
        <div className="flex justify-end">
          <div className="w-full max-w-xs space-y-2 text-sm rounded-xl border border-border bg-[#080d1a]/30 p-4 backdrop-blur-sm">
            ...
            <div className="flex justify-between gap-4">
              <span className="text-secondary">TVA ({totals.tvaLabel})</span>
        ...
        <div className="pt-4 border-t border-border text-xs text-white/45">
